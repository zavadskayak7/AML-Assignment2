...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        scores=np.exp(second_layer) / np.sum((np.exp(second_layer)),axis=1,keepdims=True) # second layer activation
       ...
       # g_wrt_a3= -(1 / y.size)* y_onehot/scores
        
       # assert g_wrt_a3.shape==scores.shape
        
        
        
        g_wrt_z3=1/y.size*(scores-y_onehot)  
        ...
        grads['W2']=np.matmul(np.transpose(first_layer_activation),g_wrt_z3)+2*reg*W2
               
        assert  grads['W2'].shape==W2.shape
        
        grads['b2']=np.sum(g_wrt_z3,axis=0)
        
        assert grads['b2'].shape==b2.shape, str(grads['b2'].shape)+ str(b2.shape)
        
    
            
        
        g_wrt_a2=np.matmul(g_wrt_z3,np.transpose(W2)) 
        
        assert g_wrt_a2.shape==first_layer_activation.shape
        
        
        
        g_wrt_z2 = g_wrt_a2*np.heaviside(first_layer_activation,0) # this is the derivative of relu.
        #practically, we for every x<0 the derivative is 0 and for all x>0, the slope(derivative) is 1

        
        assert g_wrt_z2.shape==first_layer.shape, str(g_wrt_z2.shape)+str(first_layer.shape)
        
        
        grads['W1']= np.matmul(np.transpose(X),g_wrt_z2)  +2*reg*W1    

        
        assert  grads['W1'].shape==W1.shape

        
        grads['b1']=np.sum(g_wrt_z2,axis=0)
    ...
    def train(self, X, y, X_val, y_val,
        ...
        for it in range(num_iters):
            ...
            X_combined=np.column_stack((X,y)) # y as last column of X_combined
            
            np.random.shuffle(X_combined) 
            
            X_combined=X_combined.astype('int32')
            
            X=X_combined[:,:-1] # all rows and all columns of combined,except the last one
            y=X_combined[:,-1] # all rows for the last column
