...
num_validation =1000
train = False
# these are the hyperparameters of our best model, you can use the model.ckpt file that we put in the zip with the train=False flag to avoid re-training the model
#-------------------------------------------------
...
class MultiLayerPerceptron(nn.Module):
    def __init__(self, input_size, hidden_layers, num_classes):
        ...
        layers_dict=dict()
        layers_dict['z1']=torch.nn.Linear(input_size, hidden_layers[0], bias=True)
        layers_dict['a1']=torch.nn.ReLU()
        
        for i in range(1,len(hidden_layers)):


            layers_dict['z'+str(i+1)] = torch.nn.Linear(hidden_layers[i-1], hidden_layers[i], bias=True)
            layers_dict['a'+str(i+1)]=torch.nn.ReLU()
            
        layers_dict['z_output']=torch.nn.Linear(hidden_layers[-1], num_classes, bias=True)
...
if train:
    ...
    for epoch in range(num_epochs):
        for i, (images, labels) in enumerate(train_loader):
            ...
            y_pred = model(images) # model.forward(images)
            # Loss            
        ...
        with torch.no_grad():
            ...
            for images, labels in val_loader:
                ...
                predicted = torch.max(y_pred_val, 1).indices               
